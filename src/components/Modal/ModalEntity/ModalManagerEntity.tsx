...
import { ModalManagerEntityProps } from "./types";
...
export const ModalManagerEntity = ({
  open,
  handleClose,
  onSave,
  relationEntity
}: ModalManagerEntityProps) => {
  const [label, setLabel] = useState("");

  // Função para obter o próximo ID sequencial com base na lista existente
  const getNextId = () => {
    const maxId = relationEntity.reduce(
    ...
    <Dialog onClose={handleClose} open={open} maxWidth="md" fullWidth>
      <DialogTitle>
        <Typography variant="h6" color={PALETTE.PRIMARY_MAIN}>
          Criar Nova Característica de Entidade
        </Typography>
        <ButtonIcon
          placement="top-start"
          title="Fechar"
      ...
      <Box sx={{ marginX: 5, marginY: 8, display: "flex", flexDirection: "column", gap: 3 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <InputText
            id="label"
            label="Nome da Característica Entidade"
            value={label}
