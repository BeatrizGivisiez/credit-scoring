...
export const TableListGroup = memo(
  ({
    groups,
    onViewGroup,
    onEditGroup,
    ...
    return (
      <Box sx={{ height: "auto", width: "100%" }}>
        <DataGrid
          rows={groups}
          rowCount={rowCount ?? groups.length}
          columns={columns}
          paginationMode="server" // Habilita paginação no lado do servidor
          onPaginationModelChange={(model) => {
            handleChangePagination({ page: model.page + 1 }); // Adicione +1 para ajustar a paginação do backend
            setItemsPerPage(model.pageSize); // Atualiza itemsPerPage quando o tamanho muda
          }}
